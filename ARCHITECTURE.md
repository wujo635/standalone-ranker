...
- **Identity flags now propagate through merge/import/pull for existing categories (2.3.0) — other schema edits still don't.** Originally *no* schema edit synced for a category the receiving device already had any schema for — including identity flags, and including built-in categories, since e.g. Movies always has a default schema locally from `CAT_DEFAULTS` the moment the app loads, so even a freshly-cleared device already "has a schema" and the old `if (!state.schema[c])` union check skipped adopting anything from the incoming side. Real report: a fresh import of a full baseline into a wiped device didn't carry over the source device's identity flag, because "fresh" only meant no *items*, not no *schema*. Fixed via `unionItemsAndSchema()` → `adoptIncomingIdentityFields(cat, incomingFields)`: for a category the device already knows, it adopts any field the incoming side has flagged `identity: true` that the local side hasn't — **monotonic, only ever turns identity on, never off** (removing one is the collision-risk direction the interactive Schema Editor check exists to guard; a device merely being unaware of a flag someone else already set is not grounds to strip it from them). A field entirely new to the device is added too, carrying the flag through, mirroring how new items are already unioned in unconditionally. `required`/`filterable` toggles, and any non-identity field, remain local-only — low stakes if they diverge, unlike identity which is correctness-critical for id computation, so only identity gets this treatment.
...
- **Recommended two-device rollout order, to avoid a temporary whole-category duplicate:** flagging identity rekeys *every* item in the category, not just the ones that were colliding — so if device B pulls device A's post-flag data before flagging identity locally, every item (not just the ones needing disambiguation) looks brand-new to B and gets duplicated. Flag identity on B *first* (before pulling), so B's items land on the same deterministic ids A's already use; only items A also *renamed* (the actual cleanup, e.g. "Dune (1984)" → "Dune") will still show up as a genuine duplicate on B afterward, needing a manual delete of B's stale old-titled copy.
...
| `adoptIncomingIdentityFields(cat, incomingFields)` | Monotonically adopts any `identity: true` field a synced/imported source has that this device's existing schema for `cat` doesn't — never removes one; commits the schema change only if `rekeyItemsForIdentityFields()` succeeds (2.3.0) |
