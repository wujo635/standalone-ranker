...
- `exportRecoveryBaseline()` (2.8.0) — the "something's gone wrong with sync, start over clean" recovery path. Forces a full re-pull (`pullFromFirestore({ full: true })`) merged onto this device's current rankings — every fetched match is applied via `applyNewMatches()` onto live item `elo`/`wins`/`losses` in the process — then builds the export payload and **empties its `matchLog`, stamping `_meta.matchLogCompacted: true`**, before downloading it. By that point every match's effect is already baked into item ratings, so the raw per-match log is redundant weight that would otherwise make the file grow forever with lifetime comparison count. This makes the file a single self-contained baseline reflecting this device plus everything anyone has ever uploaded to Firestore — but only correct as a **complete replacement for an empty library**, not as a merge onto one that already holds some of these items: `unionItemsAndSchema()` only ever adopts an incoming item's baked-in rating on its "no local copy at all" branch, and with `matchLog` empty there's no delta left to apply on the "item already exists" branch either, so ranking updates on any overlapping item would be silently dropped. Intended usage: wipe/reseed the Firestore collections, `clearAllData()` on every device, then Import this file everywhere. `importData()` guards this: importing a `matchLogCompacted` file into a device whose `state.items` isn't empty triggers a `confirm()` warning instead of merging silently. Only exports on a successful pull — a denied or interrupted pull aborts without producing a file, since a partial baseline that looks complete would be worse than no baseline at all. Cannot recover data that was ranked on another device but never uploaded from there; the exported file is exactly as complete as Firestore itself is.
- `cloudErrorMessage(e)` — maps Firestore error codes (`permission-denied`, `unavailable`) to a plain-language toast

**Resetting the shared baseline between User A and User B (2.8.0+):** the manual-Firestore-reset workflow this app has needed several times (see the 2.5.2/2.5.3, 2.7.3, and TODO.md's open Movies-discrepancy incidents) now has a repeatable in-app procedure instead of hand-editing Firestore and JSON exports. Use this whenever a sync bug has produced a discrepancy that isn't worth untangling fact-by-fact — pick whichever user's data you trust more as User A:

1. **Upload from every device first, starting with whichever one is *not* User A.** The recovery export can only include what's already reached Firestore — anything ranked locally and never uploaded is invisible to it and will be lost once Firestore is wiped. On User B's device: Data tab → Cloud sync → "↑ Upload rankings". Repeat for any other device that might have unsynced local comparisons.
2. **On User A's device, run the recovery export.** Data tab → "Recovery export" card → "↓ Full resync + export baseline". This pulls everything now in Firestore (including what B just uploaded in step 1) onto User A's current rankings and downloads `ranker.json`. Save this file somewhere you can get it to every device.
3. **Wipe Firestore.** In the Firebase console, delete every document under `rankers/shared` and its subcollections (`items`, `matches`, `itemDeletes`, `itemUndeletes`, `catDeletes`, `catUndeletes`) — the root doc and all six subcollections. There's no in-app button for this; it's a manual console step.
4. **Clear local data on every device, including User A's.** Data tab → "Clear all data" (`clearAllData()`). Yes, this includes the device the baseline file came from — the recovery export already contains everything that device had, so nothing is lost by clearing it, and every device needs to start from the same empty state for the next step to be correct (see the compaction caveat below).
5. **Import the baseline file on every device.** Data tab → Import → select `ranker.json` from step 2. Each device should now show identical items, ratings, and category schemas.
6. **Upload from every device to reseed Firestore.** Since Firestore was wiped in step 3, it now has nothing in it — each device needs to push its (identical, freshly-imported) state back up so future Pulls have something to fetch. Data tab → Cloud sync → "↑ Upload rankings", on every device.

**Why step 4 says "every device, including User A's":** the recovery export's `matchLog` is deliberately emptied (`_meta.matchLogCompacted: true` — see `exportRecoveryBaseline()` above) to keep the file from growing forever with lifetime comparison count. That makes it correct only as a *complete replacement* for an empty library — `unionItemsAndSchema()` never adopts an incoming rating for an item that already exists locally, so importing this file into a device that still has some of its old (possibly stale/duplicated/buggy) data would silently keep that old data's ratings instead of adopting the clean baseline's. `importData()` will refuse to proceed silently in that case (a `confirm()` warning fires when a `matchLogCompacted` file is imported into a non-empty library) — if you see that warning mid-procedure, you skipped step 4 on that device.
