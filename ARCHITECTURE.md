...
| Hidden-item markings (`item.hidden`, the ◎/◉ Library toggle) | **Untouched** — same reason as undo history above | **Lost, unconditionally, for every item** — not just ones the reset itself touched. `hidden` has been excluded from every sync transport since 1.13.1 (a personal, per-browser preference, never in a file export/import or a Firestore item doc); `clearAllDataCore()` wipes it along with everything else, and the re-populated items — freshly pulled from Firestore — were never going to carry it back regardless, the same as any ordinary first-time Pull |
