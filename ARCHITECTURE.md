  ...
  schema: {
    ...
    "Songs": {
      ...
      fields: [
        ...
        // multi: true (2.14.0) — stored as one comma-separated string ("Pop, Rock"),
        // filtered per-value not per-string. See "Multi-value fields" below.
...
3. **Apply only genuinely new, non-double-counted matches** — `mergeImport()` filters `incoming.matchLog` down to entries whose `id` isn't already in local `state.matchLog`; `applyNewMatches()` sorts them by `(ts, seq)` and applies each directly onto whatever live item ratings are already there (same `eloUpdate()` a real vote uses), never re-applying a match to a `freshlySeeded` item's side (see the "double-counting" note below). Every new match is recorded into `state.matchLog` in the same merge — none are held back.
...
An item's id is derived from its title (plus any identity field values — see `itemKey()`), so renaming a single item in the Library moves it to a new id. That move syncs as an ordinary `itemDeletes` tombstone for the **old** id carrying one extra, optional field: `renamedTo: newId`. There is no separate rename log. A tombstone without `renamedTo` is a plain delete, exactly as before.

- **On the renaming device (`saveItem()`)** the item moves via `remapItemIds({ [oldId]: newId }, { tombstones: false })`, so its local `matchLog`/`history` follow it and Undo keeps working. `tombstones: false` matters: remapping would rewrite an earlier rename's old-id tombstone into one pointing at itself. If `newId` still has an active tombstone (renaming back to an earlier title), an undelete for it is recorded first, timestamped to win. The rename tombstone's `ts` comes from `tsAfterUndeletes()` so it beats any same-millisecond undelete of the old id.
- **On a receiving device (`mergeImport()`)**, after tombstones are unioned, `makeIdResolver()` maps any id to where the item lives now: it follows `renamedTo` while the id is currently tombstoned and stops at the first live id. Only each id's latest tombstone counts (`latestPerItemId()`), so a delete after a rename wins, and a rename back can't loop because it undeleted its target. A local copy at a renamed id is moved there with `moveOrMergeItem()` — keeping *this device's* rating, matches, and history — instead of being deleted. Only a chain that ends at a plain tombstone deletes the item.
- **If the target id already holds an item on the receiving device** (it separately added the renamed-to title), the two entries are merged: wins/losses summed, rating from whichever entry has played more matches (tie: the existing one's), title/fields last-write-wins by `updatedAt`, `hidden` stays the existing one's. A match between the two entries becomes a self-match (`wid === lid`), which `applyNewMatches()` skips. Devices can end up with slightly different ELO for a merged item (same class as the replay-order drift under "Known gaps in the merge model"); win/loss totals agree.
- **Matches recorded against an old id** — by a device that voted before hearing of the rename — are resolved through `makeIdResolver()` too, both incoming ones and ones already in the local `matchLog`, so the vote lands on the renamed item.
- **The moved copy takes the renamed item's title** from the incoming item doc via last-write-wins: `unionItemsAndSchema()` now copies `title` along with `fields` from a newer incoming edit. That also fixed case-only title edits ("heat" → "Heat", same id), which previously never synced.
- **The Deleted Items view hides rename tombstones.** They aren't deletions, and undeleting the old id would bring back a stale duplicate.
- **Firestore:** the `itemDeletes` doc gains `renamedTo` (`null` on a plain delete). `tombstoneDoc()`/`tombstoneFromDoc()` are the single mapping used by both upload and pull. No new collection, no security-rule change, no `DATA_SCHEMA_VERSION` bump.
- **Older app versions** ignore `renamedTo` and treat the tombstone as a plain delete (the pre-2.17.0 behavior). Caveat: if an older device re-uploads that tombstone, the Firestore doc loses `renamedTo`, and later pulls degrade to a plain delete — no worse than before, but both devices should run 2.17.0+.
- **Not changed:** bulk identity-flag rekeys (`rekeyItemsForIdentityFields()`) still push plain tombstones. Receivers recompute those ids themselves from the adopted schema, so their ratings were never at risk.

Tests: `tests/rename.test.js` (two simulated devices; renames go through the real Library edit form).
...
### Multi-value fields (2.14.0)

Lets a field hold more than one value — e.g. a song's Genre being both "Pop" and "Rock" — while still filtering correctly on each value independently, rather than the whole comma-joined string being treated as one opaque filter option.

- A schema field can be flagged `multi: true` in the Schema Editor (`toggleMulti(i)`, same per-field-toggle-button pattern as `filterable`/`identity`). Unlike `identity`, `multi` never forces `required` and never affects `itemKey()` — it's purely a display/filter concern, no id implications, so no rekey logic is needed anywhere.
- Storage is unchanged: still a single string in `item.fields[name]`, just conventionally comma-separated ("Pop, Rock"). No `DATA_SCHEMA_VERSION` bump — same "purely additive optional flag" precedent as `filterable`/`identity`.
- `normalizeFieldValue(f, rawVal)` is the single place raw input is normalized before saving (used by both `addItem()` and `saveItem()`): splits on `,`, trims each token, drops empties, dedupes, rejoins with `", "`. Add/edit form placeholders hint `(comma-separated)` for a `multi` field.
- `fieldFilterType(cat, fieldName)` is the type-resolution entry point filtering code now uses instead of calling `inferFieldType()` directly: a `multi` field always resolves to `'multi'`, bypassing the number/string sampling heuristic entirely (a multi field's tokens are never meant to be inferred as numeric even if every token happens to look like one).
- `buildFilterFieldCards()` renders a `multi` field's filter card the same pill UI as a categorical field, but the pill options are the deduped set of individual comma-split tokens across all items, not the deduped set of raw field-value strings. `getFilteredItems()` matches with OR semantics: an item passes if *any* of its own tokens is among the selected pills.
- CSV import/export deliberately untouched: `#field` directives already don't round-trip `filterable`/`identity` either (`parsePreloadCSV()` only reads `required`) — `multi` follows the same existing gap rather than being special-cased. The comma-separated *value* itself still imports/exports fine as an opaque string (already CSV-quoted via `csvCell()` when it contains a comma).
...
- `rankers/shared/itemDeletes/{itemId}` — one tombstone doc per deleted item, keyed by `itemId` (a later delete of the same item overwrites this doc, updating `ts`): `{ itemId, ts, deviceId, title, cat, renamedTo, syncedAt }`. `renamedTo` (2.17.0) is the new id when the "delete" was really a rename, `null` otherwise — see "Item renames (2.17.0)". Mapped both ways by `tombstoneDoc()`/`tombstoneFromDoc()`
...
| Hidden-item markings (`item.hidden`, the ◎/◉ Library toggle) | **Untouched** — same reason as undo history above | **Preserved for this device's own markings (2.13.0), on a best-effort basis.** `hidden` still never travels through any sync transport (1.13.1's decision is unchanged — Firestore and file exports still never carry it) — instead, `adoptFreshBaseline()` snapshots which item ids this device had hidden *before* `clearAllDataCore()` wipes everything, then re-marks any of those ids that still exist after the fresh pull. An id that no longer exists (permanently deleted, or changed via an identity-field edit) simply doesn't get its marking back — same "id change means a different item" rule the rest of the app already follows, not a new gap. Reports a toast ("restored N hidden item(s)") only when it actually restores something; silent otherwise |
...
**Deleting a category locally without uploading first, then resetting, can silently discard someone else's already-uploaded work in that category — verified directly, not just reasoned through.** Two distinct outcomes depending on what happened elsewhere in the meantime:

- **Nothing else changed in that category:** deleting it locally (never uploaded) and then running a reset works correctly. `confirmDeleteCat()`'s local `catDeletes` tombstone is enough on its own — `resetSharedBaseline()`'s own pre-reset pull fetches Firestore's stale pre-delete copy of that category, but the same belt-and-suspenders check from 2.5.0 (`unionItemsAndSchema()` skipping any incoming item whose category is locally tombstoned) discards it before it ever reaches local state, so the reseed correctly excludes the category everywhere, with no need to have uploaded the delete first.
- **Someone else added something to that category before your reset, and you never pulled it in:** the exact same protection now actively destroys their work instead of protecting anything. Verified: device A deletes a category locally (unsynced); device B, unaware, adds a genuinely new item to that category and successfully uploads it; device A then runs "Reset shared baseline" — B's new item, which had actually reached Firestore, is silently gone from the reseed. Not "not resurrected" — discarded. The check has no way to distinguish "stale data I already knew about and rejected" from "new data someone else added after I decided to delete this"; both get excluded identically, because the tombstone says the whole category is gone, full stop. No error, no warning.

**Practical guidance, in order: Pull, then delete the category, then Reset — not delete-then-reset.** Pulling first means your local delete acts on the category's *current* state, so anything already uploaded gets absorbed and genuinely deleted along with everything else, not silently dropped later. This narrows the risk to a race condition rather than eliminating it outright: something pushed in the exact window between your Pull and your Reset is still vulnerable to the same failure. For a two-person app with no real-time locking, closing that last gap means saying so out loud ("about to delete and reset X, don't touch it") rather than relying on timing alone.
...
| `saveItem(id)` | Validates and writes edited field values back to state, saves. If the title or an identity field's value changed the item's id, moves it with `remapItemIds(..., { tombstones: false })` (matches/history follow, so Undo keeps working), undeletes the new id if it had an active tombstone (rename back), and pushes a tombstone for the old id with `renamedTo: newId` (2.17.0) — see "Item renames (2.17.0)". The duplicate-id check runs before `item` is mutated (a rejected rename leaves the item untouched), and saving with no actual change toasts "No changes" without bumping `updatedAt` (2.15.0) |
...
| `remapItemIds(idMap, opts)` | Rekeys `state.items`/`matchLog`/`history` (and `itemDeletes`, unless `opts.tombstones === false`) per `{oldId: newId}` — same remap shape as the v4 migration that introduced `itemKey()`. `rekeyItemsForIdentityFields()` remaps tombstones too (see "Item identity fields"); single-item renames (`saveItem()`, `moveOrMergeItem()`) don't |
| `makeIdResolver()` | Returns `id => currentId`: follows `renamedTo` through currently-tombstoned ids to the first live id (2.17.0). Used by `mergeImport()` to move renamed items and to resolve match ids |
| `moveOrMergeItem(fromId, toId)` | Moves a local item (with its matchLog/history references, not tombstones) to `toId`; if `toId` already exists, merges the two entries — W/L summed, rating from the entry with more matches, title/fields last-write-wins (2.17.0) |
| `tsAfterUndeletes(itemId)` | `Date.now()`, bumped past any existing undelete of `itemId`, so a same-millisecond rename tombstone still wins |
| `tombstoneDoc(t)` / `tombstoneFromDoc(d)` | The single mapping between an `itemDeletes` tombstone and its Firestore doc, used by both upload and pull; carries `renamedTo` (2.17.0) |
...
- `fieldFilterType(cat, fieldName)` (2.14.0) — the type-resolution entry point everything else calls: returns `"multi"` for a field flagged `multi: true` in the schema, else falls back to `inferFieldType()`. Keeps a `multi` field's tokens from ever being run through the number/string sampling heuristic.
- `getFilteredItems(cat, items, filters)` — applies all active filters with AND logic; numeric fields check `min`/`max`/`equals`; categorical (`"string"`) fields check multi-select values against the raw field value; `"multi"` fields split the field value on `,` and check multi-select values against each token (OR — any token matching is enough); `nonBlank` (if set, on any field type) excludes items where the field is `undefined` or `""`, independent of the type-specific condition
...
- `buildFilterFieldCards(cat, fields, catFilters)` — generates the per-field filter cards; skips fields where `filterable === false`; numeric fields show min/max/exact inputs plus a "Has value" checkbox; categorical and multi fields show checkboxes for all unique values (multi's unique values are individual comma-split tokens, not raw strings) plus the same "Has value" checkbox
- `normalizeFieldValue(f, rawVal)` (2.14.0) — for a `multi` field, splits `rawVal` on `,`, trims, drops empties, dedupes, rejoins with `", "`; a no-op for non-multi fields. Called by both `addItem()` and `saveItem()` before a field value is stored, so normalization happens in exactly one place.
...
- A field's `multi` property (2.14.0, defaults to `false`) controls whether it holds several comma-separated values (e.g. Genre) and filters per-value rather than per-string — toggled with the 🏷️ button next to filterable/identity in the Fields editor; see "Multi-value fields" above
...
- **A single-item rename (title edit, or since 2.1.0 an identity field's value edit) is fully synced as of 2.17.0** — see "Item renames (2.17.0)". History: before 2.4.0 it wasn't synced at all (duplicates on the other device); from 2.4.0 to 2.16.x it synced as a plain delete of the old id, which orphaned the renaming device's own `matchLog`/`history`, made a Firestore pull recreate the item at 1000/0W-0L on the other device (item docs carry no ratings), and deleted the item everywhere if it was later renamed back to an earlier title.
  ...
  - The rename-back trap (renaming onto an id whose earlier tombstone is still active) is closed in 2.17.0: `saveItem()` records an undelete for the target id before tombstoning the old one.
