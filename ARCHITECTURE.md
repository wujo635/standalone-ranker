  ...
  schema: {
    ...
    "Songs": {
      ...
      fields: [
        ...
        // multi: true (2.13.0) — stored as one comma-separated string ("Pop, Rock"),
...
- `fieldFilterType(cat, fieldName)` (2.13.0) — the type-resolution entry point everything else calls: returns `"multi"` for a field flagged `multi: true` in the schema, else falls back to `inferFieldType()`. Keeps a `multi` field's tokens from ever being run through the number/string sampling heuristic.
...
- `normalizeFieldValue(f, rawVal)` (2.13.0) — for a `multi` field, splits `rawVal` on `,`, trims, drops empties, dedupes, rejoins with `", "`; a no-op for non-multi fields. Called by both `addItem()` and `saveItem()` before a field value is stored, so normalization happens in exactly one place.
...
- A field's `multi` property (2.13.0, defaults to `false`) controls whether it holds several comma-separated values (e.g. Genre) and filters per-value rather than per-string — toggled with the 🏷️ button next to filterable/identity in the Fields editor; see "Multi-value fields" above
